...
// theme.ts SEVERITY/EFFORT/WINDOW labels are hardcoded pt-BR source-of-truth; the map keys must
// stay intact, so localize at the render site by mapping the key → English source string → t().
...
export function buildPtesDocx(e: Engagement, images: ChartImages): Document {
    ...
    const coverContent: (Paragraph | Table)[] = [
        ...
        kvTable([
            [t('Period'), `${e.period.start} – ${e.period.end}`],
            [t('Version'), e.version],
            [t('Author'), e.author],
            [t('Contact'), e.contact],
    ...
    children.push(body(t('Risk distribution by likelihood and impact, and concentration by category of the assessed surface.')));
    ...
    children.push(...heading(4, t('Methodology (PTES)')));
    children.push(body(t('The engagement followed the seven phases of the Penetration Testing Execution Standard (PTES).')));
    ...
    children.push(sub(t('Remediation roadmap')));
    children.push(chartPara('roadmap', 600, AlignmentType.CENTER));
    children.push(twoCharts('quadrant', 270, 'timeBars', 300));
    children.push(sub(t('Prioritized remediation items')));
    children.push(actionTable(items, primary));

    if ((e.figures?.length ?? 0) > 0) {
        children.push(...heading(7, t('Evidence')));
        children.push(body(t('Numbered evidence plan: real tool outputs and screenshots captured during execution, linked to the corresponding findings.')));
        children.push(...figuresBlocks(e));
    }
    const apx = (e.figures?.length ?? 0) > 0 ? 8 : 7;
    children.push(...heading(apx, t('Appendix')));
    children.push(sub(t('Strategic recommendations')));
                    ...
                    default: new Footer({
                        children: [
                            new Paragraph({
                                ...
                                children: [txt({ text: e.title, color: MUTED, size: 13 }), txt({ children: [`\t${t('Page')} `, PageNumber.CURRENT, ` ${t('of')} `, PageNumber.TOTAL_PAGES], color: MUTED, size: 13 })],
                            }),
                        ],
                    }),
                },
...
function kpiCardsTable(e: Engagement) {
    ...
    return new Table({
        ...
        rows: [
            new TableRow({
                children: [
                    kpiCardCell(t('Overall risk'), `${e.riskScore}`, { bg: INDIGO_SOFT, lblColor: '6663C9', numColor: INDIGO, suffix: '/100' }),
                    kpiCardCell(t('Critical'), `${crit}`, { bg: 'FBEDEC', lblColor: 'C04A40', numColor: 'E0483D' }),
                    kpiCardCell(t('Findings'), `${e.findings.length}`, { bg: PANEL, lblColor: MUTED, numColor: INK }),
                    kpiCardCell(t('Quick wins'), `${qw}`, { bg: PANEL, lblColor: MUTED, numColor: INK }),
                ],
            }),
        ],
    });
...
function findingsIndexTable(findings: Finding[], primary: string) {
    return new Table({
        ...
        borders: { ...noBorders(), insideHorizontal: { style: BorderStyle.SINGLE, size: 4, color: LINE } },
        rows: [
            new TableRow({ tableHeader: true, children: [hcell('ID', primary, 8), hcell(t('Finding'), primary, 56), hcell(t('Severity'), primary, 14), hcell('CVSS', primary, 8), hcell(t('Category'), primary, 14)] }),
...
function findingBlock(f: Finding): (Paragraph | Table)[] {
    ...
    chips.push(chipCell(`CVSS ${f.cvss.toFixed(1)}${cvssEst ? ` (${t('est.')})` : ''}`, { bg: INDIGO_SOFT, color: INDIGO_DARK }));
...
function actionTable(items: ReturnType<typeof actionItems>, primary: string) {
    ...
    return new Table({
        ...
        borders: { ...noBorders(), insideHorizontal: { style: BorderStyle.SINGLE, size: 4, color: LINE } },
        rows: [
            new TableRow({ tableHeader: true, children: [hcell(t('Window'), primary, 16), hcell('ID', primary, 7), hcell(t('Remediation action'), primary, 51), hcell(t('Effort'), primary, 11), hcell(t('Deadline'), primary, 8), hcell('QW', primary, 7)] }),
            ...sorted.map((a) =>
                new TableRow({
                    children: [
